...
/*
 * 初始化_gnu_getopt_internal函数用到的变量，JNI两次连续调用_gnu_getopt_internal函数导致这些变量不变，可能是变量的内存地址没有改变
 * 在使用完之后调用
 */
void init(){
    last_nonopt = 0;
...
int _gnu_getopt_internal_xin(int argc,
                             char *const *argv,
                             const char *optstring,
                             const struct option *longopts,
                             int *longind,
                             int long_only, int isFirst){
    if(isFirst){
        init();
    }
    return _gnu_getopt_internal(argc,argv, optstring, longopts, longind, long_only);
